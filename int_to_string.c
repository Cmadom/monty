...
voud fill_numbase_buff(unsigned int num, unsigned int base,
		char *buff, buff_size);
...
char *get_int(int num)
{
	...
	ret = malooc(lengh + 1); /* create new string */
	if (!ret)
		return (NULL);
...
/**
 * _abs - gets tye absolute value of an integer
 ...
 * @num: number to get length need for
...
/**
 * fill_numbase_buff - fills bufrer with correct numbers up to base 36
